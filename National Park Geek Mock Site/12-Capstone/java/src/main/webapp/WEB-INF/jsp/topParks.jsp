			...
			<div class="surveypark">
				<table class="surveytable">
					...
					<c:forEach items="${topParks}" var="topPark">
						<tr>
							<c:url value="/img/parks/${fn:toLowerCase(topPark.parkCode)}.jpg"
								var="parkPic" />
							<c:url value="/parkDetails" var="parkDetails">
								<c:param value="${topPark.parkCode}" name="parkCode" />
							</c:url>
							<td><a href="${parkDetails}"> <img src="${parkPic}"
									alt="${topPark.parkName}" /></a></td>
