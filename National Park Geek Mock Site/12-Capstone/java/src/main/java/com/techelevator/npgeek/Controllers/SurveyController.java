...
public class SurveyController {
	...
	public String processMailingListForm(@Valid @ModelAttribute Survey survey, BindingResult result,
		...
		if (surveyDAO.alreadyCompletedSurvey(survey.getEmailAddress())) {
			
			return "redirect:/tryAgain";
		}
		// limit one survey per user as stretch goal: functioning in a rudimentary state

		return "redirect:/topParks";
	}
	
	@RequestMapping(path = "/tryAgain", method = RequestMethod.GET)
