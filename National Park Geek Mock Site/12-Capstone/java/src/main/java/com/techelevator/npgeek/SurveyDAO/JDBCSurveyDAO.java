...
public class JDBCSurveyDAO implements SurveyDAO {
	...
	@Override
	public boolean alreadyCompletedSurvey(String emailAddress) {
		Boolean completedSurvey=false;
		String sqlTopParks ="SELECT emailaddress FROM survey_result WHERE emailaddress = ?";
		
		SqlRowSet emailInDatabase= jdbcTemplate.queryForRowSet(sqlTopParks, emailAddress); 
		while(emailInDatabase.next()) {
			if (emailAddress.contentEquals(emailInDatabase.getString("emailaddress"))) {
